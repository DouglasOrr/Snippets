   ...
   public static void main(String[] args) {
      ...
      for (int i = 0; i < numNodes; i++) {
         if(Math.random() < p_malicious)
            ...
            nodes[i] = new CraftyMaliciousNode(p_graph, p_malicious, p_txDistribution, numRounds);
         else
      ...
      // print results
      Map<Set<Transaction>, Integer> consensus = new HashMap<>();
      for (int i = 0; i < numNodes; i++) {
         ...
         System.out.println();
         System.out.println();
         if (!consensus.containsKey(transactions)) {
            consensus.put(transactions, 1);
         } else {
            consensus.put(transactions, consensus.get(transactions) + 1);
         }
      }
      ...
      int maxConsensus = 0;
      int nTransactions = 0;
      for (Map.Entry<Set<Transaction>, Integer> entry : consensus.entrySet()) {
         if (entry.getValue() > maxConsensus) {
            maxConsensus = entry.getValue();
            nTransactions = entry.getKey().size();
         }
      }
      int nCompliant = 0;
      for (Node node : nodes) {
         if (node instanceof CompliantNode_Old || node instanceof CompliantNode) {
            ++nCompliant;
         }
      }
      System.out.println(String.format("Consensus reached for %d/%d nodes (%d/%d transactions)",
              maxConsensus, nCompliant, nTransactions, numTx));
   }
